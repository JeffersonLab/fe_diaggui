...
public:
  ScopeTrace(const TGWindow *p, int traceHeight, int traceMode, int traceBitCount, int len, Color_t c)  : TGFrame(p, len*SCOPE_PIXELS_PER_BIT, traceHeight, 0, c)
  {
    ...
    for(int i=0;i<bitCount;i++)
      pTraceDataPersist[i] = new unsigned int[sampleLen];
  ...
  virtual void DoRedraw()
  {
    static TGGC *gc = 0;
    char buf[100];
    double minPersist=0.0, maxPersist=1.0;
    ...
    else if((mode & TRACE_MODE_DIGITAL) && (bitCount > 1))  // digital bus mode
    {
        ...
        for(int b = 0; b < bitCount; b++)
        {
          for(int i = 0; i < sampleLen; i++)
          {
            ...
            {
              double intensity = 255.0 * pTraceDataPersist[b][i] / maxPersist;
              int g = intensity;
              if(g > 255) g = 255;
              gc->SetForeground(g<<8); 
            }
...
public:
  Scope(const TGWindow *p, ModuleFrame *pModule) : TGCompositeFrame(p)
  {
    ...
    AddFrame(pF1 = new TGHorizontalFrame(this), new TGLayoutHints(kLHintsExpandX));
      ...
      pF1->AddFrame(pButtonPersist = new TGTextButton(pF1, new TGHotString("Pesist"), BTN_PERSIST));
        pButtonPersist->AllowStayDown(kTRUE);
        pButtonPersist->Associate(this);
      ...
      else
      {
        switch(parm1)
        {
          ...
          case BTN_SAVE:
            ...
            break;
          case BTN_PERSIST:
            for(int i=0;i<ScopeConfig.traceDesc.size();i++)
              ScopeConfig.traceFrame[i]->bPersist = pButtonPersist->IsDown();
            break;
          default:
            printf("button id %d pressed\n", (int)parm1);
            break;
