#ifndef FADC250_TrgHist2D_H
#define FADC250_TrgHist2D_H
...
class FADC250_TrgHist2D	: public TGCompositeFrame
{
public:
	FADC250_TrgHist2D(const TGWindow *p, ModuleFrame *pModule) : TGCompositeFrame(p, 400, 400)
	{
		static int inst = 0;
		int i;
...
//				pSliderUpdateTime->SetEnabled(kFALSE);
    ...
    AddFrame(pCanvas = new TRootEmbeddedCanvas("c1", this, 100, 100), new TGLayoutHints(kLHintsExpandX | kLHintsExpandY));
      pHist = new TH2I(Form("fadc250-h%d", inst), "FADC Trigger Pulse", 1024, 0, 8191, 16, 0, 16);
        pHist->GetYaxis()->SetNdivisions(16, kFALSE);
        pHist->GetYaxis()->CenterLabels(kTRUE);
		...
		for(int i = 0; i < 16; i++)
		{
            ...
            pHist->Fill(j*8, i, norm_bin);
				...
				else
				{
					...
					{
						if(buf[j] > 0x80000000)
              pHist->Fill(j*8, i, 0x7FFFFFFF);
							//pHist[i]->SetBinContent(j, 0x7FFFFFFF);
						else
              pHist->Fill(j*8, i, buf[j]);
