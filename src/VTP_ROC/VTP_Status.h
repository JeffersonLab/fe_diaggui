...
class VTP_Status	: public TGCompositeFrame
{
public:
	VTP_Status(const TGWindow *p, ModuleFrame *pModule) : TGCompositeFrame(p, 400, 400)
	{
		...
		case kC_COMMAND:
			...
			case kCM_BUTTON:
				switch(parm1)
				{
					case BTN_UPDATE:
          {
            int eb_status0  = pM->ReadReg32((volatile unsigned int *)(pM->BaseAddr+0x0A00+0x04));
            int fifo_status0 = pM->ReadReg32((volatile unsigned int *)(pM->BaseAddr+0xA1C0+0x14));
            int fifo_status1 = pM->ReadReg32((volatile unsigned int *)(pM->BaseAddr+0xA1C0+0x18));
            printf("fifo_status0 = %08X, fifo_status1 = %08X\n", fifo_status0, fifo_status1);
            printf("eb_status0 = %08X\n", eb_status0);

            int n = 0;
            for(int i=0;i<1000;i++)
            {
              int val1 = pM->ReadReg32((volatile unsigned int *)(pM->BaseAddr+0xA1C0+0x14));
              if(val1 & 0x4000)
                break;

              int val0 = pM->ReadReg32((volatile unsigned int *)(pM->BaseAddr+0xA1C0+0x8));
              printf("%3d: 0x%08X %d\n", n++, val0, (val1>>13)&0x1);
            }
