#ifndef VTP_HCALModule_H
#define VTP_HCALModule_H
...
{
public:
  VTP_HCALModule(const TGWindow *p, CrateMsgClient *pClient, unsigned int baseAddr) : ModuleFrame(p, pClient, baseAddr)
  {
    SetupRegisters();
...
#define YEAR_MAP {\
  ...
  const char *GetModuleName() { return "VTP_HCAL"; }
  const char *GetModuleFullName() { return "VXS Trigger Processor-HCAL"; }
